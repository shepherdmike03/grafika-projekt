...
namespace MyLavaRunner
{
    internal sealed class Model : IDisposable
    {
        ...
        public void Load(string path)
        {
            ...
            {
                var su = line.Split(' ', StringSplitOptions.RemoveEmptyEntries);
            ...
            for (int i = 0; i < idx.Count; i += 2)
            {
                ...
                data[o + 3] = nor.X; data[o + 4] = nor.Y; data[o + 5] = nor.Z;
            }
            ...
            const string fs =
              ...
              + "void main(){float d=max(dot(normalize(N),vec3(0,1,0)),0.25);C=vec4(d,d,d,1);}";
            ...
            {
                Add(a, list); Add(b,list); Add(c,list);
            }
            void Add(string tok, List<int> list)
            {
                var p = tok.Split('/');
